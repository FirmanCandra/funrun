...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 mb-8">
    <!-- KPI 1 -->
    <div class="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100 flex items-center">
        <div class="w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center mr-4">
        ...
        <div class="min-w-0">
            ...
            <h3 class="text-2xl sm:text-3xl font-bold text-slate-800 break-words">{{ $totalParticipants }}</h3>
        </div>
    </div>

    <!-- KPI 2 -->
    <div class="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100 flex items-center">
        <div class="w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-xl bg-green-50 text-green-600 flex items-center justify-center mr-4">
            <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </div>
        <div class="min-w-0">
            <p class="text-sm text-slate-500 font-medium mb-1">Total Revenue</p>
            <h3 class="text-2xl sm:text-3xl font-bold text-slate-800 break-words">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</h3>
        </div>
    </div>

    <!-- KPI 3 -->
    <div class="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100 flex items-center">
        <div class="w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center mr-4">
        ...
        <div class="min-w-0">
            ...
            <h3 class="text-2xl sm:text-3xl font-bold text-slate-800 break-words">{{ $ticketsSold }}</h3>
        </div>
    </div>

    <!-- KPI 4 -->
    <div class="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100 flex items-center">
        <div class="w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center mr-4">
            <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </div>
        <div class="min-w-0">
            <p class="text-sm text-slate-500 font-medium mb-1">Checked-In</p>
            <h3 class="text-2xl sm:text-3xl font-bold text-slate-800 break-words">{{ $checkedIn }}</h3>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <a href="{{ route('admin.scanner') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-colors flex items-center justify-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
            Open Scanner
        </a>
        <a href="{{ route('admin.participants') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-3 rounded-xl font-medium transition-colors text-center">
