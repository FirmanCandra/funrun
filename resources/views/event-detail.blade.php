...
<div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8 mt-10">
    ...
    <div class="glass-panel rounded-3xl overflow-hidden shadow-2xl border border-white/10 p-6 md:p-8 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-6 border-b border-white/10">
            ...
            @php($sudahTerdaftar = $myEventIds->contains($event['id']))
            <div class="shrink-0">
                @if($sudahTerdaftar)
                    {{-- Peserta yang sudah mendaftar di event ini diarahkan ke pesanannya,
                         bukan ditawari membeli lagi. --}}
                    <div class="inline-flex items-center justify-center gap-2.5 px-6 py-3 rounded-2xl font-bold text-base mb-3 w-full"
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <a href="{{ route('dashboard') }}"
                           class="flex-1 text-center px-6 py-3 rounded-2xl font-bold transition-all text-white"
                           style="background: linear-gradient(135deg, #0ea5e9, #6366f1);">
                            Lihat Pesanan
                        </a>
                        <a href="{{ route('event.register', ['event_id' => $event['id']]) }}"
                           class="flex-1 text-center px-6 py-3 rounded-2xl font-semibold text-gray-300 border border-white/10 hover:bg-white/5 transition-all">
                            Pesan Lagi
                        </a>
                    </div>
                @else
                    ...
                    @guest
                        <p class="text-xs text-gray-400 mt-3 text-center">
                            <a href="{{ route('login') }}" class="text-amber-400 hover:text-amber-300 font-semibold">Masuk</a>
                            atau
                            <a href="{{ route('register') }}" class="text-amber-400 hover:text-amber-300 font-semibold">buat akun</a>
                            dulu untuk memesan.
                        </p>
                    @endguest
                @endif
