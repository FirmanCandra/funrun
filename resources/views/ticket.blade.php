...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        @if (session('success'))
            <div
                class="rounded-btn bg-green-50 border border-green-200 px-5 py-4 text-sm text-green-800 mb-8 flex items-center gap-3">
        ...
        <div class="card overflow-hidden animate-fade-up" id="eticket">
            ...
            <div class="p-8 text-center border-b border-dashed border-line relative">
                ...
                @else
                    <div
                        class="inline-flex w-56 h-56 items-center justify-center bg-gray-50 border-2 border-dashed border-line rounded-card mb-5">
            ...
            <div class="p-8 grid grid-cols-2 gap-6">
                ...
                @if ($showAll || in_array('jersey_size', $enabledKeys))
                    <div>
                        <p class="text-xs uppercase tracking-wider text-ink-500 mb-1.5">Ukuran T-Shirt</p>
                ...
                <div>
                    ...
                    <span class="badge"
                        style="{{ $terbit ? 'background-color: #f0fdf4; color: #16a34a;' : 'background-color: #f3f4f6; color: #4b5563;' }}">
            ...
            @if(isset($enabledFields) && $enabledFields->where('is_core', false)->count() > 0)
                @php $customFieldsList = $enabledFields->where('is_core', false); @endphp
                @php $hasAnyCustomAnswer = false; @endphp
                @foreach($customFieldsList as $cf)
                    @if($participant->customAnswer($cf) !== '-') @php $hasAnyCustomAnswer = true; @endphp @endif
                @endforeach
                @if($hasAnyCustomAnswer)
                <div class="px-8 pb-6 border-t border-line pt-6">
                    <p class="text-xs uppercase tracking-wider text-ink-500 mb-4">Informasi Tambahan</p>
                    <div class="grid grid-cols-2 gap-4">
                        @foreach($customFieldsList as $cf)
                            @php $answer = $participant->customAnswer($cf); @endphp
                            @if($answer !== '-')
                            <div>
                                <p class="text-xs uppercase tracking-wider text-ink-500 mb-1.5">{{ $cf->label }}</p>
                                <p class="font-semibold text-ink-900">{{ $answer }}</p>
                            </div>
                            @endif
                        @endforeach
                    </div>
            ...
            <div class="px-8 py-5 bg-gray-50 border-t border-line text-sm text-ink-500 leading-relaxed">
                Tunjukkan QR code ini di meja registrasi untuk mengambil Pesanan.
            </div>
